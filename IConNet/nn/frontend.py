...
from .activation import nl_relu, NLReLU
...
class FeBlocks(nn.Module):
    ...
    def __init__(self, 
        ...
        blocks = [nn.Sequential(OrderedDict({
                "layer": FirConvLayer(
                    n_input_channel, n_channel[0], 
                    kernel_size=kernel_size[0], 
                    stride=stride[0], 
                    window_k=window_k[0],
                    layer_mode = layer_mode),
                "norm": CustomNormLayer(
                    name=norm_layer_name,
                    num_channels=n_channel[0])
            }))]
        blocks += [nn.Sequential(OrderedDict({
                "layer": FirConvLayer(
                    n_channel[i-1], n_channel[i], 
