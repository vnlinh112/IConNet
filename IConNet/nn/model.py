...
class M11(nn.Module):
    ...
    def forward(self, x):
        x = self.fe_blocks(x)
        # if self.pooling is not None: TODO: handle None string lol
        #     x = reduce(x, 'b c n -> b 1 c', self.pooling)
        # else: 
        x = self.seq_blocks(rearrange(x, 'b c n -> b n c'))
        x = self.cls_head(reduce(x, 'b m h -> b h', 'max'))
        return x 
