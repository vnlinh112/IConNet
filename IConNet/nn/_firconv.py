...
class IFirConv(nn.Module):
    """Interface for the FIR-based convolution layer with learnable window function & transition bandwidth.
    ...
    learnable_windows: If `True` (default), the window(s) will be parametrized 
                        with `window_k` parameters using the general consine window function [1].
    ...
    [1] Ref: scipy.signal.windows.general_cosine
...
class FirWinConv(IFirConv):
    """FIR-filter-based convolution layer using windowing method
    with learnable window function & transition bandwidth.
    """
    def __init__(self, 
            in_channels: int, out_channels: int, 
            kernel_size: int, stride: int=1, 
        ...
        super().__init__(
            ...
            learnable_windows=learnable_windows,
            shared_window=shared_window,
            window_k=window_k,
            sample_rate=sample_rate,
            filter_init=filter_init, 
            multi_resolution_count=multi_resolution_count,
...
class SincWinConv(IFirConv):
    ...
    def __init__(self, 
        ...
        filter_time = torch.arange(-n, n).repeat(in_channels,1) #* self.fs/2
        self.register_buffer('filter_time', filter_time) # 1ck    
    ...
    def _generate_filters_sincnet(self):
        """Generate FIR filters based on SincNet implementation.
        """
        min_low_ = self.min_low_hz / self.sample_rate
        low = min_low_ + torch.abs(self.lowcut_bands)
        min_band_ = self.min_band_hz / self.sample_rate
        high = low + min_band_ + torch.abs(self.bandwidths)
        f_times_t = torch.einsum(
            'hc,ck->hck', low, self.filter_time)        # hc1,1ck -> hck
        low_pass1 = torch.einsum(
            'hc,hck->hck', 2 * low, torch.sinc(2 * f_times_t)) # hcc,hck -> hck
        f_times_t = torch.einsum(
            'hc,ck->hck', high, self.filter_time)
        low_pass2 = torch.einsum(
            'hc,hck->hck', 2 * high, torch.sinc(2 * f_times_t))
        band_pass = low_pass2 - low_pass1
        max_, _ = torch.max(band_pass, dim=-1, keepdim=True)
        band_pass = band_pass / max_
        self.filters = band_pass * self.windows         # hck,k -> hck
